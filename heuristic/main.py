...
class ProducerLiteConfig:
    """Behaviour knobs.  """
    # projection window, movement build length, target ETA cap
    horizon: int = 18
    # shortlists
...
def plan_lite_waves(
    *,
    ...
    max_waves_per_turn. Returns the combined LaunchEntries (attack waves ++ regroup).
    """
    P = obs.P
    device = obs.device
    dtype = obs.ships.dtype
    ...
    # capture-floor gate at each fleet's arrival turn (defenders grow with k)
    # 6he single size must clear the defender it lands on (size >= floor_at_arr)
    # owned targets have floor 1 (reinforcement), so any positive send clears
    if K > 0:
...
def run_turn(obs_tensors: dict, *, config: ProducerLiteConfig, player_count: int, memory) -> dict:
    """full per-turn pipeline: build movement -> plan single waves + regroup -> emit
    """
...
# 4p ffa preset - differ from default 2p
